...
def main(args):
    ...
    if args.dataset_type == 'italysig':
        # ITALYSIG
        # Assuming we have a way to split train/test or use subset
        # For now, using same dataset for both but with different seeds/splits if implemented
        ...
        test_ds = ItalySigRawDataset(
            data_root=args.data_path, image_size=224,
            subset_ratio=SUBSET_RATIO, seed=43, # Different seed
            modality_mask="constellation+gaf+spectrogram"
        )
